...
import { moderateText, getModerationErrorMessage } from "@/utils/moderation";
...
router.get(
  "/debates/:debateId/arguments/with-votes",
  verifyFirebaseToken,
  validateDebateId,
  validatePagination,
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const argumentsWithVotes = argumentsList.map((arg) => {
        const hasUpvoted = arg.upvotedBy?.some(
          (id) => id.toString() === userId
        );
        const hasDownvoted = arg.downvotedBy?.some(
          (id) => id.toString() === userId
        );
        let userVote = null;
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const isInSupport = debate.supportUsers.some(
        (id) => id.toString() === userId.toString()
      );
      const isInOppose = debate.opposeUsers.some(
        (id) => id.toString() === userId.toString()
      );
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      // Moderation check for toxic words
      const moderationResult = moderateText(text.trim());
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const timeDifference =
        currentTime.getTime() - argumentCreatedAt.getTime();
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const hasUpvoted = argument.upvotedBy?.some(
        (id) => id.toString() === userId
      );
      const hasDownvoted = argument.downvotedBy?.some(
        (id) => id.toString() === userId
      );
      ...
      if (hasUpvoted) {
        argument.upvotedBy = argument.upvotedBy?.filter(
          (id) => id.toString() !== userId
        );
        argument.upvotes -= 1;
        voteChange -= 1;
      ...
      if (hasDownvoted) {
        argument.downvotedBy = argument.downvotedBy?.filter(
          (id) => id.toString() !== userId
        );
      ...
      const currentUpvoted = argument.upvotedBy?.some(
        (id) => id.toString() === userId
      );
      const currentDownvoted = argument.downvotedBy?.some(
        (id) => id.toString() === userId
      );
      let userVote = null;
